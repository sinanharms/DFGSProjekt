...
matches.path = "https://github.com/statsbomb/open-data/tree/master/data/matches/"
match.files = list()
...
for(i in 1:length(match.files)){
  matches.temp = fromJSON(match.files[[i]])
...
event.files = mgsub(event.files, "blob/", "")
event.files = paste("https://raw.githubusercontent.com", event.files, sep = "")

event.list = list()
for (i in 1:length(event.files)) {
  event.tmp = RJSONIO::fromJSON(event.files[i])
  ...
  pass.team2 = pass.index[which(unlist(lapply(pass.index, function(x) event.tmp[[x]]$team$id))==teamids[2])]
...
resp = GET("https://github.com/statsbomb/open-data/tree/master/data/matches/")

teamids = c()
#get index where find the event that talks about Starting XI
starting.xi.index = which(unlist(lapply(a, function(x) x$type$name))=="Starting XI")
starting.xi.list = list()
for (s in 1:2) {
  starting.xi.team1 = data.frame(matrix(t(unlist(a[[s]]$tactics$lineup)), ncol = 5,
                                        byrow = TRUE), stringsAsFactors = FALSE)
  colnames(starting.xi.team1) = names(unlist(a[[s]]$tactics$lineup))[1:5]
  starting.xi.team1$formation = a[[s]]$tactics$formation
  starting.xi.team1$team_id = a[[s]]$team$id
  
  teamids = c(teamids, a[[s]]$team$id)
  starting.xi.team1$team_name = a[[s]]$team$name
  starting.xi.list[[s]] = starting.xi.team1
}
pass.index = which(unlist(lapply(event.tmp, function(x) x$type$name))=="Pass")
pass.team1 = pass.index[which(unlist(lapply(pass.index, function(x) event.tmp[[x]]$team$id))==teamids[1])]
